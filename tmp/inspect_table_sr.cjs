...
async function inspect() {
    ...
    const { data: cols, error } = await supabase.rpc('inspect_table_columns', { table_name: 'service_requests' });
    
    if (error) {
        console.log('Falling back to a query to guess columns...');
        const { data, error: qError } = await supabase.from('service_requests').select('*').limit(1);
        if (qError) {
